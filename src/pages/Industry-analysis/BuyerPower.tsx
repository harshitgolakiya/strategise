...
interface BuyerPowerBoxProps {
    heading: string;
    options: string[];
}

const BuyerPowerBox: React.FC<BuyerPowerBoxProps> = (props) => {
    return (
...
const BuyerPower: React.FC = () => {
    ...
    return (
        <>
            <div className="mb-5">
                <h1 className="font-bold text-lg mb-5">Buyer Power</h1>
                <p className="text-gray-500">What's the position of our product in the customer's business?</p>
            </div>
            <div className="sm:grid grid-cols-3 gap-5">
                <BuyerPowerBox heading={heading} options={options} />
                <BuyerPowerBox heading={heading} options={options} />
                <BuyerPowerBox heading={heading} options={options} />
            </div>
        </>
